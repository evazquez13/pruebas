...
$conexion=mysqli_connect("localhost","root","root")or die ("no se pudo conectar con la base de datos");
  ...
  $select= "SELECT id_categoria,nb_empresa,tx_descripcion,tx_porcentaje,id_region,tx_estado,tx_url, X(tx_gps) as latitude, Y(tx_gps) as longitud FROM tsrh_promocion WHERE id_region='$region' and id_categoria='$categoria' and nb_empresa like '%$keyword%' or tx_descripcion like '%$keyword%' group by nb_empresa;";
  $res = mysqli_query($conexion,$select);
